...
<form action="{{ route ('getNewProduct') }}" method="POST" enctype="multipart/form-data">
    ...
    {{-- <input type="hidden" name="productEdit" value=@if ($productEdit) true @else false @endif> --}}

    <div class="input-block">
        @error('title')
        <div class="alert">{{ $message }}</div>
        @enderror
        <input type="text" name="title" id='title' value="{{ old('title') ?? ($productEdit ? $productData->title:'')}}">
    ...
    <div class="input-block">
        <textarea name="themes" id="themes" cols="30"
            rows="4">{{ old('themes')?? ($productEdit ? implode("/", $productData->themes):'') }}</textarea>
        @if(!$errors->has('themes'))<label for="themes">Тематика (через слеш - Православные/Коробельные)</label>@endif
        <div class="theme-help-block">
            @foreach ($helpTags['themes'] as $item)
            <button type="button" onclick="addWordToHelp(event)">+ <span>{{ $item }}</span></button>
            @endforeach
    ...
    <div class="input-block">
        ...
        @if(!$errors->has('images[]'))<label for="images[]">Добавьте фотографии (имя латиницей, img_01.jpg,
            img_02.jpg)</label>@endif
...
<script>
    function changeGalleryImage(event, productId, shift) {
        ...
        if (shift == 0) {
            ...
            setOkModalButton(function (){
                ajax ('/removeProductImage', {id:productId, index:index}, function (result){
            ...
            if (newIndex >=0 && newIndex < imagesArr.length) {
                ...
                ajax('/changeProductImagesArray', {newImagesArr:newImagesArr, productId:productId, removeImage: false}, function (result){
    ...
    function addWordToHelp(event){
        ...
        if (textareaElem.value !='') currentText+='/'
    ...
    document.addEventListener('DOMContentLoaded', function(){
        ...
        document.querySelectorAll('textarea').forEach(el=>{
            ...
            el.oninput = function(event){
                if (SaveButton.classList.contains('disabled')) SaveButton.classList.remove('disabled')
